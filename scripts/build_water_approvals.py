#!/usr/bin/env python3
"""Build the Ecowater certification report library from the admin API."""
...
SEARCH_KEYWORDS = ("水批", "认证证书", "水效", "检测报告", "检验报告")
...
def search_reports(certificate: str, keyword: str) -> list[dict]:
    params = {
        "keyword": keyword,
        "status": "show",
...
def classify_report(record: dict) -> str | None:
    if record.get("type") != "文档":
        return None
    title = re.sub(r"\s+", " ", str(record.get("title") or "")).strip()
    keyword = str(record.get("keyword") or "")
    if "水批报告" in title or re.search(r"水批\s*$", title):
        return "水批报告"
    if re.search(r"水效(?:报告)?\s*$", title):
        return "水效报告"
    if re.search(r"(?:检测|检验)报告", title):
        return "检测报告"
    if "产品认证证书" in keyword or re.search(r"(?:认证|CCC|NSF).*证书", title, flags=re.I):
        return "认证证书"
    return None


def merge_report_records(*groups: list[dict]) -> list[dict]:
    merged: list[dict] = []
    seen_ids: set[str] = set()
    for records in groups:
        for record in records:
            category = classify_report(record)
            content_id = str(record.get("id") or "")
            if not category or not content_id or content_id in seen_ids:
                continue
...
def get_reports(certificate: str) -> list[dict]:
    return merge_report_records(*(search_reports(certificate, keyword) for keyword in SEARCH_KEYWORDS))
...
def model_from_title(title: str) -> str:
    cleaned = re.sub(r"\s*(?:水批|水效|检测|检验)报告\s*$", "", title, flags=re.I)
    cleaned = re.sub(r"\s*水批\s*$", "", cleaned, flags=re.I)
    cleaned = re.sub(r"\s*(?:CCC|认证)证书\s*$", "", cleaned, flags=re.I)
...
def report_filename(record: dict, duplicate_names: set[str]) -> str:
    category = record.get("category") or classify_report(record) or "认证资料"
    title = str(record.get("title") or f"{category} {record.get('id')}").strip()
    if category == "水批报告":
        stem = f"{model_from_title(title)} 水批报告"
    else:
        stem = re.sub(r"\.pdf\s*$", "", title, flags=re.I).strip()
    if safe_filename(stem + ".pdf") in duplicate_names:
        stem += f"-{record.get('id')}"
    return safe_filename(stem + ".pdf")


def allocate_report_filenames(records: list[dict]) -> list[str]:
    base_names = [report_filename(record, set()) for record in records]
    name_counts = {name: base_names.count(name) for name in set(base_names)}
    used_names = {name for name, count in name_counts.items() if count == 1}
    filenames: list[str] = []

    for record, base_name in zip(records, base_names):
        if name_counts[base_name] == 1:
            filenames.append(base_name)
            continue

        candidate = report_filename(record, {base_name})
        suffix = 2
        while candidate in used_names:
            candidate = safe_filename(f"{Path(candidate).stem}-{suffix}.pdf")
            suffix += 1
...
def enrich_one(args: tuple[dict, str, Path, Path, str]) -> dict:
    ...
    category = record.get("category") or classify_report(record) or "认证资料"
    title = content.get("title") or record.get("title") or f"{category} {content_id}"
...
def build(certificate: str, out_dir: Path, workers: int) -> list[dict]:
    ...
    tasks = [
        (record, certificate, pdf_dir, thumbs_dir, filename)
        for record, filename in zip(reports, filenames)
    ]
    enriched: list[dict] = []
...
def main() -> int:
    ...
    print(f"Built {len(reports)} certification report records in {time.time() - started:.1f}s")
    return 0
